...
parser.add_argument(
    ...
    "--mode",
    required=True,
    help="specify mode: upload, download, delete, or query",
)
parser.add_argument(
    "-a",
    "--api",
    required=True,
    help="specify data commons api, e.g.: bpa, bc or genomel",
)
parser.add_argument(
    "-p",
    "--project",
    required=True,
    help="specify the project name, e.g.,: bpa-MyOrg_P0001_T1",
)
parser.add_argument(
    "-k", "--authfile", required=True, help="location of credentials.json"
)
# for upload:
parser.add_argument(
    "-f",
    ...
    required=False,
    default="nofile.tsv",
    help="meta data file to submit in tsv format",
)
parser.add_argument(
    "-l",
    "--length",
    ...
    default=100,
    required=False,
    help="length of the chunk to be submitted",
)
parser.add_argument(
    "-o",
    "--offset",
    ...
    default=0,
    required=False,
    help="specify the offset for graphQL queries. Or line offset if submitting large TSV; offset = 1000 means start at line 1000 of TSV.",
)
# for query:
parser.add_argument(
    "-n",
    ...
    required=False,
    default="none",
    help="metadata node to query or delete",
)
...
def query_api(node=args.node, project_id=args.project):
    query_txt = """query Test { %s (first:0, project_id: "%s") {id}} """ % (
        node,
        project_id,
    )
    ...
    resp = requests.post(
        gurl, json=query, headers=headers
    ).text  # Get id from submitter_id
...
if args.mode == "upload":
    ...
    with open(args.file, "rU") as file:
        ...
        print(
            "\nFound "
            + str(len(lines))
            + " lines in the tsv, including the header row.\n"
        )
    file.close
    header = ""
    data = ""
...
# 	print(dl.text)
    ...
    query_txt = """query Test { %s (first:%s, offset:%s, project_id: "%s") {id}} """ % (
        node,
        args.length,
        args.offset,
        project_id,
    ...
    for id in data["data"][node]:
        ...
        if count >= args.length:
            durl = (
                api
                + "api/v0/submission/"
                + project_id.replace("-", "/", 1)
                + "/export?format=tsv&ids="
                + list_ids[0:-1]
            )
