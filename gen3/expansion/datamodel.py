...
class Gen3Datamodel:
        ...
        if enum != "open" and "/" not in enum:
            if isinstance(enum, str) and enum.lower() in [
                ...
                try:
                    ...
                    elif (isinstance(enum_, int) or isinstance(enum_, float)) and not (
                        re.search("""[^0-9]""", enum)
                    ):
                        enum = dbl_quote(enum)
    ...
    def validate_name(self, string, name_type):
        ...
        if match:
            sys.exit(
                "Illegal character {0} found in node/property name {1}. Only lowercase letters and underscore allowed.".format(
                    match, string
                )
            )
            ...
            if node != "":
                ...
                if field != "":
                    ...
                    if enum_def == "common":
                        ...
                        enum_dict[node][field]["enumDef"][enum_val] = {
                            "$ref": [
                                dbl_quote(stripper(x)) for x in enum_def.split(",")
                            ]
                        }
    ...
    def build_properties(self, variables_df, enum_df):
        ...
        for var in var_list:
            ...
            if (
                "type" not in temp_var
                and node in enum_dict
                and field in enum_dict[node]
            ):
    ...
    def validate_links(self, link_dict, node_name):
        ...
        for key, val in len_links.items():
            if len(val) != link_len:
                flag = False
                print(
                    '\n{0} : "{1}" - {2} do not have same number of subgroups as "name" - has {3} subgroups'.format(
                        node_name, key, len(val), link_len
                    )
                )
            ...
            if key in ["group_exclusive", "group_required"]:
                for i in range(len(ref_link)):
                    if not (ref_link[i] > 0 and val[i] == 1):
                        flag = False
                        print(
                            '\n{0} : subgroups in "{1}" - {2} do not have atleast 1 or have more than 1 values for each subgroup in "name"  - has {3} in each subgroup'.format(
                                node_name, key, val, ref_link
                            )
                        )
                ...
                if val != ref_link:
                    flag = False
                    print(
                        '\n{0} : subgroups in "{1}" - {2} do not have same number of values as "name" subgroup - has {3} in each subgroup'.format(
                            node_name, key, val, ref_link
                        )
                    )
    ...
    def add_links(self, link_dict, node_name):
        ...
        if valid_links:
            for i in range(len(link_dict["name"])):
                if (
                    link_dict["group_required"][i] != [""]
                    and len(link_dict["name"][i]) > 1
                ):
                    subgroups = []

                    for l in range(len(link_dict["name"][i])):
                        subgroup = {
                            "name": link_dict["name"][i][l],
                        ...
                        link_refs[link_dict["name"][i][l]] = property_reference_setter(
                            link_dict["multiplicity"][i][l]
                        )

                    sub = {
                        "exclusive": link_dict["group_exclusive"][i][0],
                        "required": link_dict["group_required"][i][0],
                        "subgroup": subgroups,
                    }
                    links.append(sub)
        ...
        for node in nodedicts:
            ...
            for key, val in node.items():
                ...
                if key == "$schema":
                    ...
                    key == "nodeTerms"
                ):  # and terms_flag == 'et': Check this flag value if its correct
                    val_ = get_terms(val)
    ...
    def build_yamls(
        self, nodes_in_file, var_in_file, enum_in_file, in_dir, out_dir, extension
    ):  # terms_flag,
        """
        ...
        if extension == "xlsx":
            ...
            if not (node_sheet) or not (var_sheet) or not (enum_sheet):
                sys.exit(
                    "ERROR: one or more than one of the sheets (Nodes, Variable & Enum) not found, exiting the program"
                )

            nodes_df = xlsx_file.parse(
                sheet_name=node_sheet,
                index_col=None,
                header=0,
                keep_default_na=False,
                na_values=[""],
            )
            variables_df = xlsx_file.parse(
            ...
            enum_df = xlsx_file.parse(
                sheet_name=enum_sheet,
                index_col=None,
                header=0,
                keep_default_na=False,
                na_values=[""],
    ...
    def build_terms(self, terms_in_file, in_dir, out_dir, extension):
        ...
        if extension == "xlsx":
            ...
            terms_df = xlsx_file.parse(
                sheet_name=term_sheet,
                index_col=None,
                header=0,
                keep_default_na=False,
                na_values=[""],
