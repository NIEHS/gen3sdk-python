...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate a gen3-client manifest from a DCC manifest using GUID / storage-location mappings."
    )
    parser.add_argument(
        "-m",
        "--manifest",
        required=True,
        help="The manifest downloaded from DCC portal.",
        default="dcc_manifest.pdc.1581529402015.sh",
    )
    parser.add_argument(
        "-g",
        "--guid_map",
        required=False,
        help="File with PDC data file s3 location to GUID mapping.",
        default="PDC_s3_GUID_map_DCC_22199.txt",
    )
...
def read_dcc_manifest(dcc_file):
    ...
    dcc_files = [
        dcc_regex.match(i).groups()[0] for i in dcc_lines if dcc_regex.match(i)
    ]
...
def read_dcc_mapping(map_file):
    ...
    try:
        ...
        with open(map_file, "r") as mfile:
            ...
            print(
                "\t{} file records were found in the indexd file.".format(len(mapping))
            )
    except Exception as e:
...
def map_manifest(dcc_files, guid_map):
    ...
    print(
        "Found {} matching files in GUID map from the DCC manifest.".format(
            len(mani_map)
        )
    )
    return mani_map
...
def write_manifest(mani_map):
    ...
    with open(mname, "w") as manifest:
        ...
        for loc in mani_map:
            ...
            print(
                "\tGUID: {}, location: {} ({}/{})".format(object_id, loc, count, total)
            )
...
if __name__ == "__main__":
    ...
    try:
        print(
            "Reading indexd GUID / storage location mappings: '{}'".format(mapping_file)
        )
