...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Delete unmapped files uploaded with the gen3-client."
    )
    parser.add_argument(
        "-a",
        "--api",
        required=True,
        help="The data commons URL.",
        default="https://nci-crdc-demo.datacommons.io/",
    )
    parser.add_argument(
        "-u",
        "--user",
        required=True,
        help="The user's login email.",
        default="cgmeyer@uchicago.edu",
    )
    parser.add_argument(
        "-c",
        "--creds",
        required=True,
        help="The location of the credentails.json file containing the user's API keys downloaded from the /profile page of the commons.",
        default="~/Downloads/demo-credentials.json",
    )
...
if __name__ == "__main__":
    ...
    if len(guids) != 0:
        ...
        print(
            "No GUIDs found in the indexd database for uploader: {}".format(args.user)
        )
