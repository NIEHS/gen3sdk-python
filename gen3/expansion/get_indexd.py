...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Retrieve indexd for a given data commons API."
    )
    parser.add_argument(
        "-a",
        "--api",
        ...
        default="https://icgc.bionimbus.org/",
    )
    parser.add_argument(
        "-l",
        "--limit",
        type=int,
        required=False,
        help="The record limit per page use for paginating indexd records.",
        default=1000,
    )
...
def get_indexd(limit, outfile=True):
    ...
    if len(all_records) != filecount:
        print(
            "\n\nWarning: only {} of {} files in indexd downloaded!".format(
                filecount, len(all_records)
            )
        )
...
if __name__ == "__main__":
    ...
    pdc_file = (
        "/Users/christopher/Documents/Notes/ICGC/dcc_manifest.pdc.1581529402015.sh"
    )
