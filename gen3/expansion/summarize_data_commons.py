...
def get_output_name(name, extension="tsv", outdir="data_summary_reports"):
    ...
    outname = "{}/{}".format(
        outdir, outname
    )  # data_summary_reports/summary_staging_prod.tsv
    return outname
...
def summarize_dd(
    api, props_to_remove=["case_submitter_id"], nodes_to_remove=["root", "metaschema"]
):
...
def summarize_tsvs(
    ...
    for project_dir in project_dirs:  # project_dir=project_dirs[0]
        ...
        data[project_id][
            ...
            {}
        )  # currently 'nodes' is the only key in a project's dictionary, but leaving it for now in case I want to add other project specific stats
        ...
        for (
            ...
            node_regex = (
                r"^" + re.escape(project_id) + r"_([a-zA-Z0-9_]+)\.tsv$"
            ...
            if df.empty:
                ...
                print(
                    "\t\tTotal of {} records in '{}' TSV with {} properties.".format(
                        len(df), node, len(props)
                    )
                )
                ...
                for prop in props:  # prop=props[0]
                    ...
                    if nn.empty:
                        ...
                        if ptype in ["string", "enum", "boolean", "date", "array"]:
                            ...
                            data[project_id]["nodes"][node][prop]["bin_number"] = len(
                                bins
                            )
                            print(
                                "\t\t'{}.{}.{}': {}".format(
                        ...
                        # Get stats for numbers
                            ...
                            if (
                                ...
                                outliers = sorted(
                                    list(set([x for x in d if x < lower or x > upper]))
                                )
                            ...
                            print(
                                "\t\tUnhandled property type {}: {}".format(prop, ptype)
                            )
                            exit()
...
def write_commons_report(
    summary, tsv_dir, outdir=".", bin_limit=False, write_report=True, report_null=True
):
        ...
        for node in nodes:  # node=nodes[0]
            ...
            for prop in props:  # prop=props[0]
                ...
                if stats["null"] == stats["N"] and report_null is False:
                    print(
                        "\t'{}' not written to report. All records are null and 'report_null' option is set to 'False'!".format(
                            prop_id
                        )
                    )
                    ...
                    if stats["nn"] == 0:
                        ...
                        if stats["type"] in ["string", "enum", "boolean", "array"]:
                            ...
                            bins = stats["bins"]
                            if (
                                not bin_limit is False
                                and stats["bin_number"] > bin_limit
                            ):
                            ...
                            print(
                                "Unhandled property: '{}' of type '{}'!".format(
                                    prop_id, stats["type"]
                                )
                            )

                    print(
                        "\t'{}' written to report ({} total, {} null, {} non-null).".format(
                            prop_id, stats["N"], stats["null"], stats["nn"]
                        )
                    )
                    i += 1
...
def compare_commons(
    ...
    for prop_id in prop_ids:
        print(
            "({} of {} prop_ids) Comparing stats: '{}'".format(
                prop_count, total, prop_id
            )
        )
        prop_count += 1
        ...
        if len(df) == 1:
            ...
            if (
                False in same
            ):  # if any of the stats are different bw commons, add to different df and comparison df
    ...
    if len(prop_ids) == len(identical) + len(different) + len(unique):
        print(
            "All {} prop_ids in the reports were classified as having unique, identical or different data between data commons: {}.".format(
